...
window.onload = function displayWatchlist() {
    ...
    axios
      ...
      .then(response => {
        ...
        movieOutput.innerHTML += `<div class="card">
             <div class="overlay">
             <div class="addBtn"><span><i class="material-icons trash" onclick="movieSplice('${
               movie.id
             }')">delete_forever</i></span></div>
             <div class="movie">
                 <h2>${movie.title}</h2>
  ...
  if (toWatch.length == 0) {
    ...
    movieOutput.innerHTML += `<p class="infoText"> There are no movies in the watchlist. Add some now. <a href="#" onclick="openRecommendMoviesBox()"> Here are some recommendations !</a> </p>`;
  }
  // TV SHOWS
  ...
  for (let i = 0; i < toWatchTvShows.length; i++) {
    axios
      ...
      .then(response => {
        ...
        tvShowsOutput.innerHTML += `<div class="card">
                <div class="overlay">
                <div class="addBtn"><span><i class="material-icons trash" onclick="seriesSplice('${
                  series.id
                }')">delete_forever</i></span></div>
                <div class="movie">
                    <h2>${series.name}</h2>
  ...
  if (toWatchTvShows == 0) {
    // SHOW A MESSAGE IF THERE ARE NO MOVIES IN THE LIST.
    tvShowsOutput.innerHTML += `<p class="infoText"> There are no tv shows in the watchlist. Add some now. <a href="#" onclick="openRecommendTvShowsBox()"> Here are some recommendations !</a> </p>`;
  }
};
// Recommend movies
  ...
  axios
    ...
    .then(response => {
      ...
      for (let i = 0; i < movies.length; i++) {
        output += `<div class="recommended_card" onclick="movieSelected(${
          movie[i].id
        })">
...
function openRecommendTvShowsBox() {
  document.getElementById(
    'recommendedTitle'
  ).innerHTML = `Recommended TV Shows: <span class="reload"><i class="material-icons refresh" onclick="reloadRecommendedTvShows()">autorenew</i></span>`;
  recommendedBox.classList.add('recommendedBoxActive');
  recommendTvShows();
}
  ...
  axios
    .get(
      'https://api.themoviedb.org/3/discover/tv?api_key=' +
        API_KEY +
        '&language=en-US&sort_by=popularity.desc&first_air_date_year=' +
        recommendedYear +
        '&page=' +
        recommendedPage +
