      ...
      for (let i = 0; i < series.length; i++) {
        ...
        if (favoriteSeries.indexOf(id) === -1) {
          output += `
					<div class="card">
						<div class="overlay">
						<div class="addBtn"><span><i class="material-icons watch" onclick="addToList('${
              series[i].id
            }')">visibility</i></span>
						<span><i class="material-icons favorite" onclick="favorite('${
              series[i].id
            }')">favorite</i></span></div>
						<div class="movie">
							<h2>${series[i].name}</h2>
  ...
  axios
    .get(
      'https://api.themoviedb.org/3/discover/tv?api_key=' +
        ...
        year +
        '&page=1&include_null_first_air_dates=false'
    )
    .then(response => {
      let series = response.data.results;
      let output = '';
        ...
        axios
          .get(
            'https://api.themoviedb.org/3/discover/tv?api_key=' +
              ...
              '&language=en-US&sort_by=popularity.desc&first_air_date_year=' +
              year +
              '&page=1&include_null_first_air_dates=false&with_genres=' +
              selectedGenre
          )
          .then(response => {
            let series = response.data.results;
  ...
  axios
    .get(
      'https://api.themoviedb.org/3/discover/tv?api_key=' +
        ...
        pageNum +
        '&timezone=America%2FNew_York&with_genres=' +
        id +
        '&include_null_first_air_dates=false'
    )
    .then(response => {
  ...
  axios
    .get(
      'https://api.themoviedb.org/3/discover/tv?api_key=' +
        ...
        '&page=' +
        pageNum +
        '&include_null_first_air_dates=false&with_genres=' +
        genre
    )
    .then(response => {
      let series = response.data.results;
