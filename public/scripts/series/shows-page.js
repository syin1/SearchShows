...
function getShowInfo() {
  ...
  Promise.all([seriesPromise, imdbPromise, seriesCast])
    .then(([seriesResponse, imdbResponse]) => {
      ...
      let output = `
			<div class="moviePage">
					<div class="poster"><img src="http://image.tmdb.org/t/p/w300/${
            series.poster_path
          }"></div>
					<div class="info">
						<h2>${series.name}</h2>
      ...
      output += `</li>
        ...
        series.vote_count
							...
							<li><strong>Last episode to air:</strong> ${
                series.last_episode_to_air.air_date
              } "${series.last_episode_to_air.name}".</li>
      ...
      output += `</li>
        ...
        series.vote_count
                ...
                series.last_episode_to_air.air_date
						...
						<div class="buttons">
							<a href="https://www.imdb.com/title/${imdb_id}"target="_blank"> IMDB Link </a>
							<a id="addToWatchList" onclick="addToList('${
                series.id
              }')"> Add to watchlist </a>
							<a class="twitter-share-button twitter" onclick="tweet('${series.name}')"></a>
							<a onclick="goBack()"> Go back </a>
  ...
  axios
    ...
    .then(response => {
      ...
      let output = `
					<div class="video">
					<iframe width="620" height="400" src="https://www.youtube.com/embed/${
            trailer[trailerNumber].key
...
function tweet(title) {
  var strWindowFeatures =
    'location=yes,height=255,width=520,scrollbars=yes,status=yes';
  var URL =
    'https://twitter.com/intent/tweet?text=Going to watch ' +
    title +
    ' . Find something to watch @ https://pecko95.github.io/WatchSomething/&hashtags=watchSomething, whatToWatch';
